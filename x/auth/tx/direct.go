...
func (signModeDirectHandler) GetSignBytes(mode signingtypes.SignMode, data signing.SignerData, tx sdk.Tx) ([]byte, error) {
	...
	return DirectSignBytes(bodyBz, authInfoBz, data.ChainID, data.SignBlockHeight)
}
...
func DirectSignBytes(bodyBytes, authInfoBytes []byte, chainID string, signBlock uint64) ([]byte, error) {
	signDoc := types.SignDoc{
		BodyBytes:       bodyBytes,
		AuthInfoBytes:   authInfoBytes,
		ChainId:         chainID,
		SignBlockHeight: signBlock,
	}

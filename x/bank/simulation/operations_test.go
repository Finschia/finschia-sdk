...
func (suite *SimTestSuite) SetupTest() {
	...
	suite.app = app
	suite.ctx = app.BaseApp.NewContext(checkTx, ostproto.Header{})
}
...
func (suite *SimTestSuite) TestSimulateMsgSend() {
	...
	// begin a new block
	suite.app.BeginBlock(abci.RequestBeginBlock{Header: ostproto.Header{Height: suite.app.LastBlockHeight() + 1, AppHash: suite.app.LastCommitID().Hash}})
	...
	suite.Require().Equal("link1ghekyjucln7y67ntx7cf27m9dpuxxemnqk82wt", sdk.AccAddress(msg.FromAddress).String())
	suite.Require().Equal("link1p8wcgrjr4pjju90xg6u9cgq55dxwq8j7fmx8x8", sdk.AccAddress(msg.ToAddress).String())
...
func (suite *SimTestSuite) TestSimulateMsgMultiSend() {
	...
	suite.Require().Equal("link1p8wcgrjr4pjju90xg6u9cgq55dxwq8j7fmx8x8", sdk.AccAddress(msg.Inputs[1].Address).String())
	suite.Require().Equal("185121068stake", msg.Inputs[1].Coins.String())
	suite.Require().Len(msg.Outputs, 2)
	suite.Require().Equal("link1ghekyjucln7y67ntx7cf27m9dpuxxemnqk82wt", sdk.AccAddress(msg.Outputs[1].Address).String())

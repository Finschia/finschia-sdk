...
func TestJailedValidatorDelegations(t *testing.T) {
	// initial setup
	app := simapp.Setup(false)
	ctx := app.BaseApp.NewContext(false, ostproto.Header{Time: time.Unix(0, 0)})
...
func TestHandleAbsentValidator(t *testing.T) {
	...
	ctx = ctx.WithBlockHeader(ostproto.Header{Time: time.Unix(1, 0).Add(app.SlashingKeeper.DowntimeJailDuration(ctx))})
	res, err = slh(ctx, types.NewMsgUnjail(addr))
	require.NoError(t, err)
	require.NotNil(t, res)

...
func TestDispatchSubMsgErrorHandling(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"send tokens": {
			submsgID:         5,
			msg:              validBankSend,
			resultAssertions: []assertion{assertReturnedEvents(3), assertGasUsed(110200, 110300)},
		},
		"not enough tokens": {
			submsgID:    6,
			msg:         invalidBankSend,
			...
			resultAssertions: []assertion{assertGasUsed(75900, 76000), assertErrorString("codespace: sdk, code: 5")},
		},
		...
		"not enough tokens with limit": {
			...
			gasLimit:    &subGasLimit,
			// uses same gas as call without limit (note we do not charge the 40k on reply)
			resultAssertions: []assertion{assertGasUsed(76000, 76100), assertErrorString("codespace: sdk, code: 5")},
		},

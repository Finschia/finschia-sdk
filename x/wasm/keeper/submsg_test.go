...
func TestDispatchSubMsgErrorHandling(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"send tokens": {
			submsgID:         5,
			msg:              validBankSend,
			resultAssertions: []assertion{assertReturnedEvents(3), assertGasUsed(109300, 109400)},
		},
		"not enough tokens": {
			submsgID:    6,
			msg:         invalidBankSend,
			...
			resultAssertions: []assertion{assertGasUsed(75500, 75600), assertErrorString("codespace: sdk, code: 5")},
		},
		...
		"send tokens with limit": {
			...
			resultAssertions: []assertion{assertReturnedEvents(3), assertGasUsed(109400, 109500)},
		},

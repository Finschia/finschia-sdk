...
func (msg *MsgDelegate) FromOperations(ops []*rosettatypes.Operation) (sdk.Msg, error) {
	...
	for _, op := range ops {
		if strings.HasPrefix(op.Amount.Value, "-") {
			...
			err = sdk.ValidateAccAddress(op.Account.Address)
			if err != nil {
				return nil, err
			}
			delAddr = sdk.AccAddress(op.Account.Address)
			continue
		}
		...
		err = sdk.ValidateValAddress(op.Account.SubAccount.Address)
		if err != nil {
			return nil, err
		}
		valAddr = sdk.ValAddress(op.Account.SubAccount.Address)
...
func (msg *MsgBeginRedelegate) FromOperations(ops []*rosettatypes.Operation) (sdk.Msg, error) {
	...
	for _, op := range ops {
		if strings.HasPrefix(op.Amount.Value, "-") {
			...
			srcValAddr = sdk.ValAddress(op.Account.SubAccount.Address)
			continue
		}
		...
		destValAddr = sdk.ValAddress(op.Account.SubAccount.Address)

...
func initValidators(t testing.TB, power int64, numAddrs int, powers []int64) (*simapp.SimApp, sdk.Context, []sdk.AccAddress, []sdk.ValAddress, []types.Validator) {
	...
	for i, power := range powers {
		vs[i] = teststaking.NewValidator(t, addrs[i].ToValAddress(), pks[i])
...
func TestSetValidator(t *testing.T) {
	...
	valPubKey := PKs[0]
	valAddr := sdk.BytesToValAddress(valPubKey.Address())
...
func TestUpdateBondedValidatorsDecreaseCliff(t *testing.T) {
	...
	for valIdx, status := range expectedValStatus {
		...
		val, _ := app.StakingKeeper.GetValidator(ctx, sdk.ValAddress(valAddr))
...
func TestSlashToZeroPowerRemoved(t *testing.T) {
	...
	app.StakingKeeper.Slash(ctx, sdk.BytesToConsAddress(PKs[0].Address()), 0, 100, sdk.OneDec())
	// apply TM updates
...
func TestValidatorBasics(t *testing.T) {
	...
	resVal, found = app.StakingKeeper.GetValidatorByConsAddr(ctx, sdk.BytesToConsAddress(PKs[0].Address()))
	require.True(t, found)
	assert.True(ValEq(t, validators[0], resVal))
...
func TestGetValidatorSortingUnmixed(t *testing.T) {
	...
	for i, amt := range amts {
		validators[i] = teststaking.NewValidator(t, addrs[i].ToValAddress(), PKs[i])
...
func TestGetValidatorSortingMixed(t *testing.T) {
	...
	val0, found := app.StakingKeeper.GetValidator(ctx, addrs[0].ToValAddress())
	require.True(t, found)
	val1, found := app.StakingKeeper.GetValidator(ctx, addrs[1].ToValAddress())
	require.True(t, found)
	val2, found := app.StakingKeeper.GetValidator(ctx, addrs[2].ToValAddress())
	require.True(t, found)
	val3, found := app.StakingKeeper.GetValidator(ctx, addrs[3].ToValAddress())
	require.True(t, found)
	val4, found := app.StakingKeeper.GetValidator(ctx, addrs[4].ToValAddress())
	require.True(t, found)
...
func TestGetValidatorsEdgeCases(t *testing.T) {
	...
	for i, power := range powers {
		moniker := fmt.Sprintf("val#%d", int64(i))
		validators[i] = newMonikerValidator(t, addrs[i].ToValAddress(), PKs[i], moniker)
...
func TestValidatorBondHeight(t *testing.T) {
	...
	// initialize some validators into the state
	var validators [3]types.Validator
	validators[0] = teststaking.NewValidator(t, sdk.BytesToValAddress(PKs[0].Address()), PKs[0])
	validators[1] = teststaking.NewValidator(t, addrs[1].ToValAddress(), PKs[1])
	validators[2] = teststaking.NewValidator(t, addrs[2].ToValAddress(), PKs[2])
...
func TestUpdateValidatorCommission(t *testing.T) {
	app, ctx, _, addrVals := bootstrapValidatorTest(t, 1000, 20)
	ctx = ctx.WithBlockHeader(ostproto.Header{Time: time.Now().UTC()})

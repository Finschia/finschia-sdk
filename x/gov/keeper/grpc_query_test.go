	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				expRes = &types.QueryVoteResponse{Vote: types.Vote{ProposalId: proposal.ProposalId, Voter: addrs[0].String(), Option: types.OptionEmpty, Options: []types.WeightedVoteOption{{Option: types.OptionAbstain, Weight: sdk.MustNewDecFromStr("1.0")}}}}
			},
			true,
		},
		{
...
>>>>>>> 29c7a469f (refactor: Bring back deprecated proto fields to `v1beta1` (#9534))
				}
				accAddr1 := sdk.AccAddress(votes[0].Voter)
				accAddr2 := sdk.AccAddress(votes[1].Voter)

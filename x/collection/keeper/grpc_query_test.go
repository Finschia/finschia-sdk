...
func (s *KeeperTestSuite) TestQueryRoot() {
	// empty request
	_, err := s.queryServer.Root(s.ctx, nil)
	s.Require().Error(err)
	...
	for name, tc := range testCases {
		s.Run(name, func() {
			...
			res, err := s.queryServer.Root(s.ctx, req)
			if !tc.valid {
				s.Require().Error(err)
				return
			}
...
func (s *KeeperTestSuite) TestQueryHasParent() {
	// empty request
	_, err := s.queryServer.HasParent(s.ctx, nil)
	s.Require().Error(err)
	...
	for name, tc := range testCases {
		s.Run(name, func() {
			...
			res, err := s.queryServer.HasParent(s.ctx, req)
			if !tc.valid {
				s.Require().Error(err)
				return
			}
...
func (s *KeeperTestSuite) TestQueryParent() {
	// empty request
	_, err := s.queryServer.Parent(s.ctx, nil)
	s.Require().Error(err)
	...
	for name, tc := range testCases {
		s.Run(name, func() {
			...
			res, err := s.queryServer.Parent(s.ctx, req)
			if !tc.valid {
				s.Require().Error(err)
				return
			}
...
func (s *KeeperTestSuite) TestQueryChildren() {
	// empty request
	_, err := s.queryServer.Children(s.ctx, nil)
	s.Require().Error(err)
	...
	for name, tc := range testCases {
		s.Run(name, func() {
			...
			res, err := s.queryServer.Children(s.ctx, req)
			if !tc.valid {
				s.Require().Error(err)
				return
			}

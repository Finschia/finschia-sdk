...
func prepareCollectionTokens(ctx sdk.Context, t *testing.T) {
	...
	require.NoError(t, keeper.IssueCNFT(ctx, addr1, defaultSymbol))
	require.NoError(t, keeper.IssueCNFT(ctx, addr1, defaultSymbol2))
	...
	require.NoError(t, err)
	require.NoError(t, keeper.MintCNFT(ctx, addr1, types.NewNFT(collection2, defaultName, defaultTokenType, defaultTokenURI, addr1)))
	collection, err = keeper.GetCollection(ctx, defaultSymbol)
	require.NoError(t, err)
	require.NoError(t, keeper.GrantPermission(ctx, addr1, addr2, types.NewMintPermission(defaultSymbol+defaultTokenType)))
	require.NoError(t, keeper.MintCNFT(ctx, addr2, types.NewNFT(collection, defaultName, defaultTokenType, defaultTokenURI, addr2)))
	require.NoError(t, keeper.IssueCFT(ctx, addr1, types.NewFT(collection, defaultName, defaultTokenURI, sdk.NewInt(1), true), sdk.NewInt(defaultAmount)))
}

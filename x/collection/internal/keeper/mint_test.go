...
func TestKeeper_MintFT(t *testing.T) {
	...
	require.EqualError(t, keeper.MintFT(ctx, defaultContractID, addr2, addr2, types.NewCoins(types.NewCoin(defaultTokenIDFT, sdk.NewInt(10)))), sdkerrors.Wrapf(types.ErrTokenNoPermission, "Account: %s, Permission: %s", addr2.String(), types.NewMintPermission()).Error())

...
func TestKeeper_MintNFT(t *testing.T) {
	...
	require.EqualError(t, keeper.MintNFT(ctx, addr2, types.NewNFT(defaultContractID, defaultTokenID1, "sword", defaultMeta, addr1)), types.ErrTokenNoPermission(types.DefaultCodespace, addr2, types.NewMintPermission(defaultContractID)).Error())
	require.EqualError(t, keeper.MintNFT(ctx, addr1, types.NewNFT(wrongContractID, defaultTokenID1, "sword", defaultMeta, addr1)), types.ErrTokenTypeNotExist(types.DefaultCodespace, wrongContractID, defaultTokenType).Error())
	require.EqualError(t, keeper.MintNFT(ctx, addr1, types.NewNFT(defaultContractID, wrongTokenID, "sword", defaultMeta, addr1)), types.ErrTokenTypeNotExist(types.DefaultCodespace, defaultContractID, defaultTokenType2).Error())

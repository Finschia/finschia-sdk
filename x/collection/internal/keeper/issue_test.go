...
func TestKeeper_IssueFT(t *testing.T) {
	...
	require.EqualError(t, keeper.IssueFT(ctx, addr1, addr1, types.NewFT(wrongContractID, defaultTokenIDFT, defaultName, defaultMeta, sdk.NewInt(1), true), sdk.NewInt(defaultAmount)), sdkerrors.Wrapf(types.ErrCollectionNotExist, "ContractID: %s", wrongContractID).Error())
...
func TestKeeper_IssueNFT(t *testing.T) {
	...
	require.EqualError(t, keeper.IssueNFT(ctx, types.NewBaseTokenType(wrongContractID, defaultTokenType, defaultName, defaultMeta), addr1), sdkerrors.Wrapf(types.ErrCollectionNotExist, "ContractID: %s", wrongContractID).Error())

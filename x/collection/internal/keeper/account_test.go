...
func TestKeeper_GetAccountSupply(t *testing.T) {
	...
	{
		_, err := keeper.GetBalance(ctx, defaultSymbol, defaultTokenIDFT, addr1)
		require.Error(t, err)
	}
	t.Log("Set tokens to addr1")
	{
		_, err := keeper.AddCoins(ctx, defaultSymbol, addr1, types.NewCoins(types.NewCoin(defaultTokenIDFT, sdk.NewInt(defaultAmount))))

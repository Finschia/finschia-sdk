...
func TestDefaultGenesisState(t *testing.T) {
	...
	require.NoError(t, foundation.ValidateGenesis(*gs, addressCodec))
...
func TestValidateGenesis(t *testing.T) {
		...
		"proposals": {
			data: foundation.GenesisState{
				...
				Proposals: []foundation.Proposal{
					*foundation.Proposal{
						Id:                1,
						Proposers:         []string{bytesToString(addrs[1])},
						FoundationVersion: 1,
		...
		"invalid foundation tax": {
			data: foundation.GenesisState{
				Params: foundation.Params{
					FoundationTax: math.LegacyNewDec(2),
				},
				Foundation: foundation.DefaultFoundation(),
			},
		...
		"non empty proposals with outsourcing decision policy": {
			data: foundation.GenesisState{
				...
				Proposals: []foundation.Proposal{
					*foundation.Proposal{
						Id:                1,
						Proposers:         []string{bytesToString(addrs[0])},
						FoundationVersion: 1,
					}.WithMsgs([]sdk.Msg{testdata.NewTestMsg()}),
				},
			},
	...
	for name, tc := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			...
			err = foundation.ValidateGenesis(unmarshalled, addressCodec)
			if !tc.valid {
				require.Error(t, err)
				return
			}

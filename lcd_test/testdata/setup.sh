...
echo $SEND_TX_HASH
sleep 3s
PUBLISH_TX_HASH=$(echo ${PASSWORD} | ./build/linkcli --home ${HOME} tx token publish jack lcdtoken lcdtoken 100000000 true --chain-id ${CHAIN_ID} --yes -b block | awk '/txhash.*/{print $2}')

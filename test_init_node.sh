...
for ((i = 0; i < N; i++))
  do
    ...
    if [ "`uname`" = "Linux" ]; then
      ...
      awk '!x{x=sub("enable = false","enable = true")}1' "${CHAIN_DIR}"/config/app.toml > "${CHAIN_DIR}"/config/tmp.toml
      mv "${CHAIN_DIR}"/config/tmp.toml "${CHAIN_DIR}"/config/app.toml
    else
      sed -i '' 's#"tcp://127.0.0.1:26657"#"tcp://0.0.0.0:'"${RPC_PORT}"'"#g' ${CHAIN_DIR}/config/config.toml
      sed -i '' 's#"tcp://0.0.0.0:26656"#"tcp://0.0.0.0:'"${P2P_PORT}"'"#g' ${CHAIN_DIR}/config/config.toml
      sed -i '' 's#"localhost:6060"#"localhost:'"${PROF_PORT}"'"#g' ${CHAIN_DIR}/config/config.toml
    ...
    RPC_PORT=$((RPC_PORT + 2))
    P2P_PORT=$((P2P_PORT + 2))
    PROF_PORT=$((PROF_PORT + 1))
    GRPC_PORT=$((GRPC_PORT + 2))
    GRPC_WEB_PORT=$((GRPC_WEB_PORT + 2))
  done

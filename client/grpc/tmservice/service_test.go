...
func (s IntegrationTestSuite) TestQueryValidatorSetByHeight() {
	...
	// no pagination rest
	_, err = rest.GetRequest(fmt.Sprintf("%s/lbm/base/ostracon/v1/validatorsets/%d", val.APIAddress, 1))
	s.Require().NoError(err)

	// rest query with pagination
	restRes, err := rest.GetRequest(fmt.Sprintf("%s/lbm/base/ostracon/v1/validatorsets/%d?pagination.offset=%d&pagination.limit=%d", val.APIAddress, 1, 0, 1))

...
func (s IntegrationTestSuite) TestQueryNodeInfo() {
	...
	restRes, err := rest.GetRequest(fmt.Sprintf("%s/cosmos/base/ostracon/v1beta1/node_info", val.APIAddress))
	s.Require().NoError(err)
...
func (s IntegrationTestSuite) TestQuerySyncing() {
	...
	restRes, err := rest.GetRequest(fmt.Sprintf("%s/cosmos/base/ostracon/v1beta1/syncing", val.APIAddress))
	s.Require().NoError(err)
...
func (s IntegrationTestSuite) TestQueryLatestBlock() {
	...
	restRes, err := rest.GetRequest(fmt.Sprintf("%s/cosmos/base/ostracon/v1beta1/blocks/latest", val.APIAddress))
	s.Require().NoError(err)
...
func (s IntegrationTestSuite) TestQueryBlockByHeight() {
	...
	restRes, err := rest.GetRequest(fmt.Sprintf("%s/cosmos/base/ostracon/v1beta1/blocks/%d", val.APIAddress, 1))
	s.Require().NoError(err)
...
func (s IntegrationTestSuite) TestQueryLatestValidatorSet() {
	...
	_, err = rest.GetRequest(fmt.Sprintf("%s/cosmos/base/ostracon/v1beta1/validatorsets/latest", val.APIAddress))
	s.Require().NoError(err)

	// rest request with pagination
	restRes, err := rest.GetRequest(fmt.Sprintf("%s/cosmos/base/ostracon/v1beta1/validatorsets/latest?pagination.offset=%d&pagination.limit=%d", val.APIAddress, 0, 1))
	s.Require().NoError(err)
...
func (s IntegrationTestSuite) TestQueryValidatorSetByHeight() {
	...
	// no pagination rest
	_, err = rest.GetRequest(fmt.Sprintf("%s/cosmos/base/ostracon/v1beta1/validatorsets/%d", val.APIAddress, 1))
	s.Require().NoError(err)

	// rest query with pagination
	restRes, err := rest.GetRequest(fmt.Sprintf("%s/cosmos/base/ostracon/v1beta1/validatorsets/%d?pagination.offset=%d&pagination.limit=%d", val.APIAddress, 1, 0, 1))

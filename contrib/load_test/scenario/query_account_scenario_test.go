...
func TestQueryAccountScenario_GenerateTarget(t *testing.T) {
	// Given Mock Server
	...
	targets, numTargets, err := accountScenario.GenerateTarget(keyWallet, 0)
	require.NoError(t, err)

	require.Equal(t, 1, numTargets)
	require.Equal(t, "GET", (*targets)[0].Method)
	require.Equal(t, fmt.Sprintf("%s/auth/accounts/%s", server.URL, keyWallet.Address().String()), (*targets)[0].URL)
}

...
type TxMultiMintNFTScenario struct {
...
func (s *TxMultiMintNFTScenario) GenerateStateSettingMsgs(masterKeyWallet *wallet.KeyWallet,
...
func (s *TxMultiMintNFTScenario) GenerateTarget(keyWallet *wallet.KeyWallet, walletIndex int) (*[]*vegeta.Target, int, error) {
	...
	for i := 0; i < s.config.MsgsPerTxLoadTest; i++ {
		...
		msgs[i] = collection.NewMsgMintNFT(from, contractID, to, params...)
	}
